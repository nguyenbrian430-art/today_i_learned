...
async function loadFacts() {
  const res = await fetch(
    "https://meoutyizrsytvhhkmdwi.supabase.co/rest/v1/facts",
    {
      headers: {
        apikey: "sb_publishable_tgUmOxO3NWzf1hcQ0Qkgzw_GL-dItLs",
        authorization: "Bearer sb_publishable_tgUmOxO3NWzf1hcQ0Qkgzw_GL-dItLs",
      },
    }
  );
...
const times10 = [2, 4, 6, 8].map(function (x) {
  return x * 10;
});
console.log(times10);

const times10Two = [2, 4, 6, 8].map((x) => x * 10);
console.log(times10Two);
// "..." is spread operator that spreads out each value in an array
...
const factAges = initialFacts.map((x) => calcFactAge(x.createdIn));
